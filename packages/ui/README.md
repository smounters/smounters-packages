...
Для массовых действий (снять с публикации, назначить, удалить пачкой) включается отдельно:

```tsx
<DataTable
  code="orders.list"
  getRowId={(o) => o.id}   // обязателен: id выбранных строк берутся отсюда
  enableSelection
  onSelectionChange={setSelectedIds}
  selectionResetSignal={resetTick}  // необязательно: увеличить, чтобы снять выделение снаружи
  …
/>
```

Колонка с галочками рисуется первой, её нельзя перетащить или спрятать. **Выделение живёт в пределах
страницы**: любые новые данные (другая страница, смена фильтра, перезапрос) его снимают — иначе
массовое действие применится к строкам, которых пользователь уже не видит. В настройках оно не
хранится: это состояние экрана, а не преференс.

Подписи галочек — `labels.table.selectAll` и `labels.table.selectRow`.
